...
SgfNode::SgfNode() : props(NULL), move(0, 0, D_NONE, C_EMPTY) {}
SgfNode::SgfNode(const SgfNode& other) : props(NULL), move(0, 0, D_NONE, C_EMPTY) {
  if(other.props != NULL)
    props = new map<string, vector<string>>(*(other.props));
  move = other.move;
}
SgfNode::SgfNode(SgfNode&& other) noexcept : props(NULL), move(0, 0, D_NONE, C_EMPTY) {
  props = other.props;
  other.props = NULL;
  move = other.move;
}
...
static int parseSgfDirection(char c) {
  if(c >= 'a' && c <= 'd')
    return (int)c - (int)'a';
  if(c >= 'A' && c <= 'D')
    return (int)c - (int)'A';
...
static MoveNoBSize parseSgfLocNoSize(const string& s, Player pla) {
  if(s.length() == 0)
    return MoveNoBSize(COORD_MAX, COORD_MAX, D_NONE, pla);
  if(s.length() != 3)
    propertyFail("Invalid location: " + s);

  int x = parseSgfCoord(s[0]);
  int y = parseSgfCoord(s[1]);
  int dir = parseSgfDirection(s[2]);

  if(x < 0 || y < 0 || x >= COORD_MAX || y >= COORD_MAX)
    propertyFail("Invalid location: " + s);
  return MoveNoBSize(x, y, dir, pla);
}

static Spot parseSgfSpot(const string& s, int xSize, int ySize) {
  if(s.length() != 2)
    propertyFail("Invalid location: " + s);
...
static void parseSgfLocRectangle(const string& s, int xSize, int ySize, int& x1, int& y1, int& x2, int& y2) {
  ...
  if(
    x1 < 0 || x1 >= xSize || y1 < 0 || y1 >= ySize || x2 < 0 || x2 >= xSize || y2 < 0 || y2 >= ySize || x1 > x2 ||
    y1 > y2)
...
static Loc parseSgfLoc(const string& s, int xSize, int ySize) {
  ...
  assert(s.length() == 3);
  return Loc(parseSgfSpot(s.substr(0, 2), xSize, ySize), parseSgfDirection(s[2]));
}

static void writeSgfSpot(ostream& out, Spot spot, int xSize, int ySize) {
  ...
  if(spot == Board::NULL_LOC)
    return;
  int x = Location::getX(spot, xSize);
  int y = Location::getY(spot, xSize);
  const char* chars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
  out << chars[x];
  out << chars[y];
}

static void WriteSgfDirection(ostream& out, Direction dir) {
  if(dir == D_NONE)
    return;
...
static void writeSgfLoc(ostream& out, Loc loc, int xSize, int ySize) {
  writeSgfSpot(out, loc.spot, xSize, ySize);
  WriteSgfDirection(out, loc.dir);
}
...
void SgfNode::accumPlacements(vector<Placement>& moves, int xSize, int ySize) const {
  if(props == NULL)
    return;
    ...
    for(size_t i = 0; i < len; i++) {
      ...
      for(int x = x1; x <= x2; x++) {
        for(int y = y1; y <= y2; y++) {
          ...
          moves.push_back(Placement(spot, color));
        }
      }
    }
  };
...
void SgfNode::accumMoves(vector<Move>& moves, int xSize, int ySize) const {
  if(move.pla == C_BLACK) {
    if(move.x >= xSize || move.y >= ySize)
      propertyFail("Move out of bounds: " + Global::intToString(move.x) + "," + Global::intToString(move.y));
    moves.push_back(Move(Loc(Location::getSpot(move.x, move.y, xSize), move.dir), move.pla));
  }
  if(props != NULL && contains(*props, "B")) {
    const vector<string>& b = map_get(*props, "B");
    size_t len = b.size();
    for(size_t i = 0; i < len; i++) {
      Loc loc = parseSgfLoc(b[i], xSize, ySize);
  ...
  if(props != NULL && contains(*props, "W")) {
    ...
    size_t len = w.size();
    for(size_t i = 0; i < len; i++) {
      Loc loc = parseSgfLoc(w[i], xSize, ySize);
...
XYSize Sgf::getXYSize() const {
  ...
  if(!nodes[0]->hasProperty("SZ"))
    return XYSize(5, 5);  // Some SGF files don't specify, in that case assume 5
...
int Sgf::getWinLen() const {
  checkNonEmpty(nodes);
  int winLen = 0;
  if(nodes[0]->hasProperty("WLL")) {
    bool suc = Global::tryStringToInt(nodes[0]->getSingleProperty("WLL"), winLen);
    if(!suc)
      propertyFail("Could not parse win length in sgf");
  }
}
...
void Sgf::getPlacements(vector<Placement>& moves, int xSize, int ySize) const {
  moves.clear();
  checkNonEmpty(nodes);
...
void Sgf::getMovesHelper(vector<Move>& moves, int xSize, int ySize) const {
  ...
  for(int i = 0; i < nodes.size(); i++) {
    if(i > 0 && nodes[i]->hasPlacements())
      propertyFail(
        "Found stone placements after the root, game records that are not simply ordinary play not currently "
        "supported");
...
void Sgf::loadAllUniquePositions(
  ...
  bool allowGameOver,
  Rand* rand,
  vector<PositionSample>& samples) const {
  std::function<void(PositionSample&, const BoardHistory&, const string&)> f =
    [&samples](PositionSample& sample, const BoardHistory& hist, const string& comments) {
      (void)hist;
      (void)comments;
...
void Sgf::iterAllUniquePositions(
  ...
  bool allowGameOver,
  Rand* rand,
  std::function<void(PositionSample&, const BoardHistory&, const std::string&)> f) const {
  XYSize size = getXYSize();
  int winLen = getWinLen();
  int xSize = size.x;
  int ySize = size.y;
...
void Sgf::iterAllPositionsHelper(
  ...
  for(size_t i = 0; i < nodes.size(); i++) {
    ...
    if(nodes[i]->hasPlacements()) {
      ...
      if(buf.size() > 0) {
        ...
        for(size_t j = 0; j < buf.size(); j++) {
          if(board.colors[buf[j].spot] != C_EMPTY && buf[j].pla == C_EMPTY)
            netStonesAdded--;
          if(board.colors[buf[j].spot] == C_EMPTY && buf[j].pla != C_EMPTY)
            netStonesAdded++;
        }
    ...
    // Handle actual moves
    movebuf.clear();
    nodes[i]->accumMoves(movebuf, xSize, ySize);

    for(size_t j = 0; j < movebuf.size(); j++) {
      bool legal = hist.isLegal(board, movebuf[j].loc, movebuf[j].pla);
      if(legal)
        hist.makeBoardMoveAssumeLegal(board, movebuf[j].loc, movebuf[j].pla);
      else {
        ostringstream trace;
        for(size_t s = 0; s < variationTraceNodesBranch.size(); s++) {
          trace << "forward " << variationTraceNodesBranch[s].first << " ";
        ...
        throw StringError(
          "Illegal move in " + fileName + " effective turn " + Global::int64ToString(j + hist.initialTurnNumber) +
          " move " + GameIO::locToString(movebuf[j].loc, board) + " SGF trace (branches 0-indexed): " + trace.str());
      }
      if(hist.moveHistory.size() > 0x3FFFFFFF)
        throw StringError("too many moves in sgf");
      nextPla = getOpp(movebuf[j].pla);
      samplePositionHelper(
        board,
        hist,
        nextPla,
...
void Sgf::samplePositionHelper(
  ...
  while(turnsAgoToSnap < 5) {
    ...
    if(
      turnsAgoToSnap > 0 && hist.moveHistory[hist.moveHistory.size() - turnsAgoToSnap - 1].pla ==
                              hist.moveHistory[hist.moveHistory.size() - turnsAgoToSnap].pla)
      break;
  ...
  sampleBuf.hintLoc = Loc(Board::NULL_LOC, D_NONE);
...
string Sgf::PositionSample::toJsonLine(const Sgf::PositionSample& sample) {
  ...
  data["winLen"] = sample.board.win_len;
  ...
  for(size_t i = 0; i < sample.moves.size(); i++)
    moveLocs.push_back(GameIO::locToString(sample.moves[i].loc, sample.board));
  for(size_t i = 0; i < sample.moves.size(); i++)
  ...
  data["hintLoc"] = GameIO::locToString(sample.hintLoc, sample.board);
...
Sgf::PositionSample Sgf::PositionSample::ofJsonLine(const string& s) {
  ...
  try {
    ...
    sample.board = Board::parseBoard(xSize, ySize, winLen, data["board"].get<string>(), '/');
    ...
    if(moveLocs.size() != movePlas.size() * 2)
      throw StringError("moveLocs.size() != movePlas.size() * 2");
    for(size_t i = 0; i < moveLocs.size(); i += 2) {
      Loc moveLoc = GameIO::parseLoc(moveLocs[i] + " " + moveLocs[i + 1], sample.board);
      Player movePla = GameIO::parsePlayer(movePlas[i / 2]);
    ...
    if(
      hintLocStr == "" || hintLocStr == "''" || hintLocStr == "\"\"" || hintLocStr == "null" ||
      hintLocStr == "'null'" || hintLocStr == "\"null\"")
      sample.hintLoc = Loc(Board::NULL_LOC, D_NONE);
    else
      sample.hintLoc = GameIO::parseLoc(data["hintLoc"].get<string>(), sample.board);
...
Sgf::PositionSample Sgf::PositionSample::getColorFlipped() const {
  ...
  Board newBoard(other.board.x_size, other.board.y_size, other.board.win_len);
  for(int y = 0; y < other.board.y_size; y++) {
    for(int x = 0; x < other.board.x_size; x++) {
      Spot spot = Location::getSpot(x, y, other.board.x_size);
      if(other.board.colors[spot] == C_BLACK || other.board.colors[spot] == C_WHITE) {
        bool suc = newBoard.setStone(spot, getOpp(other.board.colors[spot]));
        assert(suc);
        (void)suc;
      }
    }
...
Sgf::PositionSample Sgf::PositionSample::previousPosition(double newWeight) const {
  ...
  if(other.moves.size() > 0) {
    ...
    other.hintLoc = Loc(Board::NULL_LOC, D_NONE);
...
bool Sgf::PositionSample::isEqualForTesting(const Sgf::PositionSample& other, bool checkNumCaptures, bool checkSimpleKo)
  const {
...
static void sgfFail(const string& msg, const string& str, int entryPos, int pos) {
  throw IOError(
    msg + " (entryPos " + Global::intToString(entryPos) + "):" + " (pos " + Global::intToString(pos) + "):\n" + str);
}
...
static char peekSgfTextChar(const string& str, int& pos, int& newPos) {
  ...
  if(newPos >= str.length())
    sgfFail("Unexpected end of str", str, newPos);
  return str[newPos++];
}
...
static bool maybeParseProperty(SgfNode* node, const string& str, int& pos) {
  ...
  while(true) {
    ...
    if(node->move.pla == C_EMPTY && key == "B") {
      node->move = parseSgfLocNoSize(parseTextValue(str, pos), P_BLACK);
    } else if(node->move.pla == C_EMPTY && key == "W") {
      node->move = parseSgfLocNoSize(parseTextValue(str, pos), P_WHITE);
    } else {
      if(node->props == NULL)
...
static Sgf* maybeParseSgf(const string& str, int& pos) {
  ...
  Sgf* sgf = new Sgf();
  try {
    while(true) {  // Note: B and W properties are captured and others are saved in props
...
CompactSgf::CompactSgf(const Sgf* sgf)
  : fileName(sgf->fileName), rootNode(), placements(), moves(), xSize(), ySize(), winLen(), depth() {
  XYSize size = sgf->getXYSize();
  winLen = sgf->getWinLen();
  xSize = size.x;
  ySize = size.y;
  depth = sgf->depth();
  hash = sgf->hash;
...
CompactSgf::CompactSgf(Sgf&& sgf) : fileName(), rootNode(), placements(), moves(), xSize(), ySize(), depth() {
  XYSize size = sgf.getXYSize();
  xSize = size.x;
  ySize = size.y;
  depth = sgf.depth();
...
void CompactSgf::playMovesTolerant(Board& board, Player& nextPla, BoardHistory& hist, int64_t turnIdx) const {
  ...
  for(int64_t i = 0; i < turnIdx; i++) {
    bool isLegal = hist.isLegal(board, moves[i].loc, moves[i].pla);
    if(isLegal)
      hist.makeBoardMoveAssumeLegal(board, moves[i].loc, moves[i].pla);
    else
      throw StringError(
        "Illegal move in " + fileName + " turn " + Global::int64ToString(i) + " move " +
        GameIO::locToString(moves[i].loc, board));
    nextPla = getOpp(moves[i].pla);
  }
}

void CompactSgf::setupBoardAndHistAssumeLegal(Board& board, Player& nextPla, BoardHistory& hist, int64_t turnIdx)
  const {
  setupInitialBoardAndHist(board, nextPla, hist);
  playMovesAssumeLegal(board, nextPla, hist, turnIdx);
}

void CompactSgf::setupBoardAndHistTolerant(Board& board, Player& nextPla, BoardHistory& hist, int64_t turnIdx) const {
  setupInitialBoardAndHist(board, nextPla, hist);
...
string WriteSgf::gameResultNoSgfTag(const BoardHistory& hist) {
  if(!hist.isGameFinished)
    ...
    string ret;
    if(hist.winner == C_BLACK)  // if the game ends normally, then only write the winner and a plus.
      ret = "B+";
...
void WriteSgf::writeSgf(
  ...
  int winLen = initialBoard.win_len;
  out << "(;FF[4]GM[Coffee]";
  if(xSize == ySize)
    out << "SZ[" << xSize << "]";
  else
  ...
  for(int y = 0; y < ySize; y++) {
    for(int x = 0; x < xSize; x++) {
      Spot spot = Location::getSpot(x, y, xSize);
      if(initialBoard.colors[spot] == C_BLACK) {
        if(!hasAB) {
          out << "AB";
          hasAB = true;
        }
        out << "[";
        writeSgfSpot(out, spot, xSize, ySize);
        out << "]";
      }
    }
  }
  ...
  for(int y = 0; y < ySize; y++) {
    for(int x = 0; x < xSize; x++) {
      Spot spot = Location::getSpot(x, y, xSize);
      if(initialBoard.colors[spot] == C_WHITE) {
        if(!hasAW) {
          out << "AW";
          hasAW = true;
        }
  ...
  if(gameData != NULL) {
    ...
    if(gameData->usedInitialPosition != 0)
      out << ","
          << "usedInitialPosition=" << gameData->usedInitialPosition;
    ...
    if(gameData->bTimeUsed > 0 || gameData->wTimeUsed > 0) {
      out << ","
          << "bTimeUsed=" << gameData->bTimeUsed;
      out << ","
          << "wTimeUsed=" << gameData->wTimeUsed;
    }
    out << "]";
  ...
  BoardHistory hist(board, endHist.initialPla);
  for(size_t i = 0; i < endHist.moveHistory.size(); i++) {
    ...
    bool isResignMove = endHist.isGameFinished && endHist.isResignation && endHist.winner == getOpp(pla) &&
                        i + 1 == endHist.moveHistory.size();
