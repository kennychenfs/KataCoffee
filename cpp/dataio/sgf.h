...
struct Sgf {
  ...
  // Hashes are used to filter out "identical" positions when loading many files from different SGFs that may have
  // overlapping openings, etc. The hashes are not guaranteed to correspond to position hashes, or anything else
  // external to this function itself. May raise an exception on illegal moves or other SGF issues, only partially
  // appending things on to the boards and hists. If rand is provided, will randomize order of iteration through the
  // SGF. If hashParent is true, will determine uniqueness by the combination of parent hash and own hash.
  void loadAllUniquePositions(
    ...
    bool allowGameOver,
    Rand* rand,
    std::vector<PositionSample>& samples) const;
...
namespace WriteSgf {
  ...
  // indicate the index of the first turn that should be used for training data. (0 means the whole SGF, 1 means
  // skipping black's first move, etc). If valueTargets is not NULL, also write down after each move the MCTS values
  // following that search move. If overideFinishedScore is not NAN and the game is finished and the result is a score
  // (rather than noresult or resign), set this score instead, from white's perspective.
  void writeSgf(
    std::ostream& out,
