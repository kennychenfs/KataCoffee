...
class NNEvaluator {
 ...
 private:
  ...
  int numEvalsToAwaken;  // Current number of things waitingForFinish that should be woken up. Used to avoid spurious
                         // wakeups.
  ...
  // An array of NNResultBuf** of length numResultBufss, each NNResultBuf** is an array of NNResultBuf* of length
  // maxNumRows. If a full resultBufs array fills up, client threads can move on to fill up more without waiting.
  // Implemented basically as a circular buffer.
