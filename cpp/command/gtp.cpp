...
struct GTPEngine {
      ...
      callback = [args, pla, this](const Search* search) {
        ...
        for(int i = 0; i < buf.size(); i++) {
          ...
          if(args.showMovesOwnershipStdev) {
            ...
            movesOwnershipAverageAndStdev =
              search->getAverageAndStandardDeviationTreeOwnership(perspective, data.node, data.symmetry);
          ...
          if(args.showMovesOwnership) {
            ...
            for(int y = 0; y < board.y_size; y++) {
              for(int x = 0; x < board.x_size; x++) {
                ...
                out
                  << " "
                  << movesOwnership[pos];  // perspective already handled by getAverageAndStandardDeviationTreeOwnership
              }
            }
          }
          if(args.showMovesOwnershipStdev) {
  ...
  void computeAnticipatedWinnerAndScore(Player& winner, double& finalWhiteMinusBlackScore) {
    ...
    // Human-friendly score or incomplete game score estimation
      ...
      double lead =
        PlayUtils::computeLead(bot->getSearchStopAndWait(), NULL, board, hist, pla, numVisits, OtherGameProperties());
...
int MainCmds::gtp(const vector<string>& args) {
  ...
  while(getline(cin, line)) {
    ...
    // TODO someday maybe make this fully general, like in the analysis engine.
      ...
      int stones;
      if(
        pieces.size() != 3 || !GameIO::tryParsePlayer(pieces[0], pla) || !Global::tryStringToDouble(pieces[1], time) ||
      ...
      if(!parsed) {
        ...
        if(pieces.size() == 2) {
          parsed = false;
          if(
            Global::tryStringToDouble(pieces[0], policyOptimism) && isnan(policyOptimism) && policyOptimism >= 0.0 &&
            policyOptimism <= 1.0) {
            parsed = true;
          }
        }
      ...
      if(parsed) {
        ...
        if(boardSizeX != boardSizeY) {
          responseIsError = true;
          response = "Current board size is " + Global::intToString(boardSizeX) + "x" +
                     Global::intToString(boardSizeY) + ", no built-in benchmarks for rectangular boards";
        } else {
          CompactSgf* sgf = NULL;
          try {
          ...
          if(sgf != NULL) {
            ...
            try {
              PlayUtils::BenchmarkResults results = PlayUtils::benchmarkSearchOnPositionsAndPrint(
                params, sgf, 10, engine->nnEval, baseline, secondsPerGameMove, printElo);
              (void)results;
            } catch(const StringError& e) {
              responseIsError = true;
              response = e.what();
            ...
            if(sgf != NULL) {
              delete sgf;
              // Act of benchmarking will write to stdout with a newline at the end, so we just need one more newline
              // ourselves to complete GTP protocol.
              suppressResponse = true;
              cout << endl;
            }
          }
