...
class KataHelpOutput : public TCLAP::StdOutput {
 ...
 public:
  KataHelpOutput(int numBuiltIn, int shortUsageLimit)
    : TCLAP::StdOutput(), numBuiltInArgs(numBuiltIn), shortUsageArgLimit(shortUsageLimit) {}

  virtual ~KataHelpOutput() {}

  void setShortUsageArgLimit(int n) { shortUsageArgLimit = n; }
  ...
  virtual void _shortUsage(TCLAP::CmdLineInterface& _cmd, ostream& os) const {
    ...
    int lowerLimit =
      shortUsageArgLimit < 0 ? 0 : std::max(0, (int)argVec.size() - numBuiltInArgs - 1 - shortUsageArgLimit + 1);
...
void KataGoCommandLine::addConfigFileArg(
  const string& defaultCfgFileName,
  const string& exampleConfigFile,
  bool required) {
...
string KataGoCommandLine::getModelFile() const {
  ...
  if(modelFile.empty()) {
    ...
    try {
      ...
      throw StringError(
        string("'-model MODELFILENAME.bin.gz was not provided but encountered error searching for default: ") +
        err.what());
    }
    if(pathForErrMsg == "")
      pathForErrMsg = getDefaultModelPathForHelp();
    throw StringError(
      "-model MODELFILENAME.bin.gz was not specified to tell KataGo where to find the neural net model, and default "
      "was not found at " +
      pathForErrMsg);
  }
  return modelFile;
...
vector<string> KataGoCommandLine::getConfigFiles() const {
  ...
  if(configFiles.empty() && !defaultConfigFileName.empty()) {
    ...
    try {
      ...
      throw StringError(
        string("'-config CONFIG_FILE_NAME.cfg was not provided but encountered error searching for default: ") +
        err.what());
    }
    if(pathForErrMsg == "")
      pathForErrMsg = getDefaultConfigPathForHelp(defaultConfigFileName);
    throw StringError(
      "-config CONFIG_FILE_NAME.cfg was not specified to tell KataGo where to find the config, and default was not "
      "found at " +
      pathForErrMsg);
  }
  return configFiles;
  ...
  if(overrideConfigArg != NULL) {
    ...
    for(const string& overrideConfig: overrideConfigs) {
      if(overrideConfig != "") {
        ...
        // HACK to avoid a common possible conflict - if we specify some of the rules options on one side, the other
        // side should be erased.
