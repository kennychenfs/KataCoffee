...
Loc Location::getMirrorSpot(Loc loc, int x_size, int y_size) {
  if(loc == Board::NULL_LOC || loc == Board::PASS_LOC)
    return loc;
  return getSpot(x_size - 1 - getX(loc, x_size), y_size - 1 - getY(loc, x_size), x_size);
}

Loc Location::getCenterSpot(int x_size, int y_size) {
  if(x_size % 2 == 0 || y_size % 2 == 0)
    return Board::NULL_LOC;
  return getSpot(x_size / 2, y_size / 2, x_size);
}

Loc Location::getCenterSpot(const Board& b) {
...
int Board::getNumLibertiesAfterPlay(Loc loc, Player pla, int max) const {
  ...
  // Next, walk through all stones of all surrounding groups we would connect with and count liberties, avoiding
  // overlap.
...
bool Board::isLegal(Loc loc, Player pla, bool isMultiStoneSuicideLegal) const {
  if(pla != P_BLACK && pla != P_WHITE)
    return false;
  return loc == PASS_LOC || (loc >= 0 && loc < MAX_ARR_SIZE && (colors[loc] == C_EMPTY) && !isKoBanned(loc) &&
...
bool Board::wouldBeCapture(Loc loc, Player pla) const {
  if(colors[loc] != C_EMPTY)
    return false;
  Player opp = getOpp(pla);
  FOREACHADJ(Loc adj = loc + ADJOFFSET; if(colors[adj] == opp) {
    if(getNumLiberties(adj) == 1)
      return true;
  });
...
bool Board::isAdjacentToPla(Loc loc, Player pla) const {
  FOREACHADJ(Loc adj = loc + ADJOFFSET; if(colors[adj] == pla) return true;);
  return false;
}
...
// Does this connect two pla distinct groups that are not both pass-alive and not within opponent pass-alive area
// either?
...
int Board::getNumImmediateLiberties(Loc loc) const {
  int num_libs = 0;
  if(colors[loc + ADJ0] == C_EMPTY)
    num_libs++;
  if(colors[loc + ADJ1] == C_EMPTY)
    num_libs++;
  if(colors[loc + ADJ2] == C_EMPTY)
    num_libs++;
  if(colors[loc + ADJ3] == C_EMPTY)
    num_libs++;
...
void Board::mergeChains(Loc loc1, Loc loc2) {
  ...
  while(true) {
    ...
    FOREACHADJ(Loc adj = loc + ADJOFFSET; if(colors[adj] == C_EMPTY && !isLibertyOf(adj, head1)) numNewLiberties++;);
...
Loc Board::rebuildChainHelper(Loc head, Loc tailTarget, Loc loc, Player pla) {
  ...
  if(colors[adj0] == C_EMPTY && !isLibertyOf(adj0, head))
    numHeadLibertiesToAdd++;
  if(colors[adj1] == C_EMPTY && !isLibertyOf(adj1, head))
    numHeadLibertiesToAdd++;
  if(colors[adj2] == C_EMPTY && !isLibertyOf(adj2, head))
    numHeadLibertiesToAdd++;
  if(colors[adj3] == C_EMPTY && !isLibertyOf(adj3, head))
    numHeadLibertiesToAdd++;
  ...
  // Recursively add stones around us.
  Loc nextTailTarget = loc;
  if(colors[adj0] == pla && chain_head[adj0] != head)
    nextTailTarget = rebuildChainHelper(head, nextTailTarget, adj0, pla);
  if(colors[adj1] == pla && chain_head[adj1] != head)
    nextTailTarget = rebuildChainHelper(head, nextTailTarget, adj1, pla);
  if(colors[adj2] == pla && chain_head[adj2] != head)
    nextTailTarget = rebuildChainHelper(head, nextTailTarget, adj2, pla);
  if(colors[adj3] == pla && chain_head[adj3] != head)
    nextTailTarget = rebuildChainHelper(head, nextTailTarget, adj3, pla);
  return nextTailTarget;
}
...
void Board::changeSurroundingLiberties(Loc loc, Player pla, int delta) {
  ...
  if(
    colors[adj2] == pla && !(colors[adj0] == pla && chain_head[adj0] == chain_head[adj2]) &&
    !(colors[adj1] == pla && chain_head[adj1] == chain_head[adj2]))
    chain_data[chain_head[adj2]].num_liberties += delta;
  if(
    colors[adj3] == pla && !(colors[adj0] == pla && chain_head[adj0] == chain_head[adj3]) &&
...
bool Board::hasLibertyGainingCaptures(Loc loc) const {
  ...
  Loc cur = loc;
  do {
    FOREACHADJ(Loc adj = cur + ADJOFFSET; if(colors[adj] == opp) {
      Loc head = chain_head[adj];
      if(chain_data[head].num_liberties == 1)
        return true;
    });
...
bool Board::searchIsLadderCaptured(Loc loc, bool defenderFirst, vector<Loc>& buf) {
  ...
  // Stack for the search. These point to lists of possible moves to search at each level of the stack, indices refer to
  // indices in [buf].
  int stackSize =
    x_size * y_size * 3 / 2 + 1;  // A bit bigger due to paranoia about recaptures making the sequence longer.
  ...
  while(true) {
    // if(print) cout << ": " << stackIdx << " " << moveListCur[stackIdx] << " " << moveListStarts[stackIdx] << " " <<
    // moveListLens[stackIdx] << " " << returnValue << " " << returnedFromDeeper << endl;
    ...
    if(moveListCur[stackIdx] == -1) {
      ...
      if(isDefender) {
        ...
        if(libs0 == 0 && libs1 == 0 && wouldBeKoCapture(buf[start], opp) && wouldBeKoCapture(buf[start + 1], opp)) {
          if(
            getNumLibertiesAfterPlay(buf[start], pla, 3) <= 2 &&
            getNumLibertiesAfterPlay(buf[start + 1], pla, 3) <= 2) {
    ...
    // if(print) cout << "play " << Location::getX(move,x_size) << " " << Location::getY(move,x_size) << " " << p <<
    // endl;
...
void Board::calculateAreaForPla(
  ...
  for(int i = 0; i < numRegions; i++) {
    ...
    // Mark pass alive territory and pass-alive stones and large empty regions bordered only own pass-alive stones
    // unconditionally These should be mutually exclusive with these same regions but for the opponent, so this is safe.
    // We need to mark unconditionally since we WILL sometimes overwrite points of the opponent's color marked earlier,
    // in the case that the opponent was marking unsafeBigTerritories and marked an empty spot surrounded by a pass-dead
    // group.
    bool shouldMark =
      numInternalSpacesMax2[i] <= 1 && !bordersNonPassAlivePlaByHead[head] && atLeastOnePla;  // internal space can be 2
    shouldMark =
      shouldMark || (safeBigTerritories && !containsOpp[i] && !bordersNonPassAlivePlaByHead[head] && atLeastOnePla);
    if(shouldMark) {
      Loc cur = head;
      do {
        result[cur] = pla;
      ...
      // Mark unsafeBigTerritories only if the opponent didn't already claim the very stones we're using to surround it
      // as pass-dead and therefore the whole thing as pass-alive-territory.
      ...
      if(basicArea[loc] != C_EMPTY && !isSeki[loc]) {
        if(
          ...
          while(queueHead != queueTail) {
            ...
            FOREACHADJ(Loc adj = nextLoc + ADJOFFSET; if(basicArea[adj] == pla && !isSeki[adj]) {
              isSeki[adj] = true;
              queue[queueTail++] = adj;
            });
          }
    ...
    for(int x = 0; x < x_size; x++) {
      ...
      if(basicArea[loc] != C_EMPTY && !isSeki[loc] && result[loc] != basicArea[loc]) {
        ...
        while(queueHead != queueTail) {
          ...
          // Look all around it, floodfill
          FOREACHADJ(Loc adj = nextLoc + ADJOFFSET; if(basicArea[adj] == pla && result[adj] != basicArea[adj]) {
...
bool Board::isAdjacentToPlaHead(Player pla, Loc loc, Loc plaHead) const {
  FOREACHADJ(Loc adj = loc + ADJOFFSET; if(colors[adj] == pla && chain_head[adj] == plaHead) return true;);
  return false;
}
...
bool Board::countEmptyHelper(bool* emptyCounted, Loc initialLoc, int& count, int bound) const {
  ...
  while(numExpanded < numLeft) {
    ...
    FOREACHADJ(Loc adj = loc + ADJOFFSET; if(colors[adj] == C_EMPTY && !emptyCounted[adj]) {
      count += 1;
