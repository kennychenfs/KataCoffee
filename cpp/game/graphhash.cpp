...
Hash128 GraphHash::getGraphHash(Hash128 prevGraphHash, const BoardHistory& hist, Player nextPlayer) {
  const Board& board = hist.getRecentBoard(0);
  Loc prevMoveLoc =
    hist.moveHistory.size() <= 0 ? Loc(Board::NULL_LOC, D_NONE) : hist.moveHistory[hist.moveHistory.size() - 1].loc;
...
Hash128 GraphHash::getGraphHashFromScratch(const BoardHistory& histOrig, Player nextPlayer) {
  ...
  for(size_t i = 0; i < histOrig.moveHistory.size(); i++) {
    ...
    hist.makeBoardMove(board, histOrig.moveHistory[i].loc, histOrig.moveHistory[i].pla);
  }
  assert(board.pos_hash == histOrig.getRecentBoard(0).pos_hash);
