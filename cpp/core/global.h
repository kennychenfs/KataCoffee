...
class asserted_unreachable : public std::exception {
  const char* what() const throw() final { return "BUG? Reached asserted-unreachable point of the code!"; }
};
...
struct StringError : public std::exception {
  std::string message;
  StringError(const char* m) : exception(), message(m) {}
  StringError(const std::string& m) : exception(), message(m) {}

  const char* what() const throw() final { return message.c_str(); }
};

// Common exception for IO
struct IOError : public StringError {
  IOError(const char* msg) : StringError(msg){};
  IOError(const std::string& msg) : StringError(msg){};
};
// Exception for config parsing
struct ConfigParsingError final : public IOError {
  ConfigParsingError(const char* msg) : IOError(msg){};
  ConfigParsingError(const std::string& msg) : IOError(msg){};
};
// Common exception for parameter values
struct ValueError final : public StringError {
  ValueError(const char* msg) : StringError(msg){};
  ValueError(const std::string& msg) : StringError(msg){};
};
// Common exception for command line argument handling
struct CommandError final : public StringError {
  CommandError(const char* msg) : StringError(msg){};
  CommandError(const std::string& msg) : StringError(msg){};
};

// Named pairs and triples of data values
#define STRUCT_NAMED_SINGLE(A, B, C) \
  struct C { \
    A B; \
    inline C() : B() {} \
    inline C(A s_n_p_arg_0) : B(s_n_p_arg_0) {} \
  }
#define STRUCT_NAMED_PAIR(A, B, C, D, E) \
  struct E { \
    A B; \
    C D; \
    inline E() : B(), D() {} \
    inline E(A s_n_p_arg_0, C s_n_p_arg_1) : B(s_n_p_arg_0), D(s_n_p_arg_1) {} \
    inline bool operator==(const E& other) const { \
      return B == other.B && D == other.D; \
    } \
    inline bool operator!=(const E& other) const { \
      return !(*this == other); \
    } \
  }
#define STRUCT_NAMED_TRIPLE(A, B, C, D, E, F, G) \
  struct G { \
    A B; \
    C D; \
    E F; \
    inline G() : B(), D(), F() {} \
    inline G(A s_n_p_arg_0, C s_n_p_arg_1, E s_n_p_arg_2) : B(s_n_p_arg_0), D(s_n_p_arg_1), F(s_n_p_arg_2) {} \
  }
#define STRUCT_NAMED_QUAD(A, B, C, D, E, F, G, H, I) \
  struct I { \
    A B; \
    C D; \
    E F; \
    G H; \
    inline I() : B(), D(), F(), H() {} \
    inline I(A s_n_p_arg_0, C s_n_p_arg_1, E s_n_p_arg_2, G s_n_p_arg_3) \
      : B(s_n_p_arg_0), D(s_n_p_arg_1), F(s_n_p_arg_2), H(s_n_p_arg_3) {} \
  }
