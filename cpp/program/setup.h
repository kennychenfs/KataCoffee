...
namespace Setup {
  ...
  std::vector<SearchParams> loadParams(ConfigParser& cfg, setup_for_t setupFor);
  SearchParams loadSingleParams(ConfigParser& cfg, setup_for_t setupFor);

  Player parseReportAnalysisWinrates(ConfigParser& cfg, Player defaultPerspective);

  // Returns true if the user's config specified the size, false if it did not. If false, does not set
  // defaultBoardXSizeRet or defaultBoardYSizeRet.
  ...
  std::string loadHomeDataDirOverride(ConfigParser& cfg);
