...
vector<SearchParams> Setup::loadParams(ConfigParser& cfg, setup_for_t setupFor) {
  ...
  for(int i = 0; i < numBots; i++) {
    ...
    if(cfg.contains("fpuParentWeightByVisitedPolicy" + idxStr))
      params.fpuParentWeightByVisitedPolicy = cfg.getBool("fpuParentWeightByVisitedPolicy" + idxStr);
    else if(cfg.contains("fpuParentWeightByVisitedPolicy"))
      params.fpuParentWeightByVisitedPolicy = cfg.getBool("fpuParentWeightByVisitedPolicy");
    else
      params.fpuParentWeightByVisitedPolicy = (setupFor != SETUP_FOR_DISTRIBUTED);
    if(params.fpuParentWeightByVisitedPolicy) {
      if(cfg.contains("fpuParentWeightByVisitedPolicyPow" + idxStr))
        params.fpuParentWeightByVisitedPolicyPow =
          cfg.getDouble("fpuParentWeightByVisitedPolicyPow" + idxStr, 0.0, 5.0);
      else if(cfg.contains("fpuParentWeightByVisitedPolicyPow"))
        params.fpuParentWeightByVisitedPolicyPow = cfg.getDouble("fpuParentWeightByVisitedPolicyPow", 0.0, 5.0);
      else
    ...
    if(cfg.contains("useGraphSearch" + idxStr))
      params.useGraphSearch = cfg.getBool("useGraphSearch" + idxStr);
    else if(cfg.contains("useGraphSearch"))
      params.useGraphSearch = cfg.getBool("useGraphSearch");
    else
      params.useGraphSearch = (setupFor != SETUP_FOR_DISTRIBUTED);
    if(cfg.contains("graphSearchRepBound" + idxStr))
      params.graphSearchRepBound = cfg.getInt("graphSearchRepBound" + idxStr, 3, 50);
    else if(cfg.contains("graphSearchRepBound"))
      params.graphSearchRepBound = cfg.getInt("graphSearchRepBound", 3, 50);
    else
      params.graphSearchRepBound = 11;
    if(cfg.contains("graphSearchCatchUpLeakProb" + idxStr))
      params.graphSearchCatchUpLeakProb = cfg.getDouble("graphSearchCatchUpLeakProb" + idxStr, 0.0, 1.0);
    else if(cfg.contains("graphSearchCatchUpLeakProb"))
      params.graphSearchCatchUpLeakProb = cfg.getDouble("graphSearchCatchUpLeakProb", 0.0, 1.0);
    else
      params.graphSearchCatchUpLeakProb = 0.0;
    // if(cfg.contains("graphSearchCatchUpProp"+idxStr)) params.graphSearchCatchUpProp =
    // cfg.getDouble("graphSearchCatchUpProp"+idxStr, 0.0, 1.0); else if(cfg.contains("graphSearchCatchUpProp"))
    // params.graphSearchCatchUpProp = cfg.getDouble("graphSearchCatchUpProp", 0.0, 1.0); else
    // params.graphSearchCatchUpProp = 0.0;
    ...
    if(cfg.contains("nodeTableShardsPowerOfTwo" + idxStr))
      params.nodeTableShardsPowerOfTwo = cfg.getInt("nodeTableShardsPowerOfTwo" + idxStr, 8, 24);
    else if(cfg.contains("nodeTableShardsPowerOfTwo"))
      params.nodeTableShardsPowerOfTwo = cfg.getInt("nodeTableShardsPowerOfTwo", 8, 24);
    else
...
bool Setup::loadDefaultBoardXYSize(
  ...
  const int defaultBoardYSize = cfg.contains("defaultBoardYSize")  ? cfg.getInt("defaultBoardYSize", 2, Board::MAX_LEN)
                                : cfg.contains("defaultBoardSize") ? cfg.getInt("defaultBoardSize", 2, Board::MAX_LEN)
                                                                   : -1;
  const int defaultBoardWinLen =
    cfg.contains("defaultBoardWinLen") ? cfg.getInt("defaultBoardWinLen", 2, Board::MAX_LEN) : -1;
  if((defaultBoardXSize == -1) != (defaultBoardYSize == -1))
    logger.write(
      "Warning: Config specified only one of defaultBoardXSize or defaultBoardYSize and no other board size "
      "parameter, "
      "ignoring it");

  if(defaultBoardXSize == -1 || defaultBoardYSize == -1 || defaultBoardWinLen == -1) {
    return false;
  }
  defaultBoardXSizeRet = defaultBoardXSize;
  defaultBoardYSizeRet = defaultBoardYSize;
  defaultBoardWinLenRet = defaultBoardWinLen;
  return true;
}
