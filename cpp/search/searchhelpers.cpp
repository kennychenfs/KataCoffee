...
std::shared_ptr<NNOutput>* Search::maybeAddPolicyNoiseAndTemp(SearchThread& thread, bool isRoot, NNOutput* oldNNOutput) const {
  ...
  if(rootHint != Board::NULL_LOC) {
    const float propToMove = 0.02f;
    int pos = getPos(rootHint);
    if(noisedPolicyProbs[pos] >= 0) {
...
double Search::getResultUtility(double winLossValue) const {
  return (
...
void Search::getSelfUtilityLCBAndRadius(const SearchNode& parent, const SearchNode* child, int64_t edgeVisits, Action move, double& lcbBuf, double& radiusBuf) const {
  ...
  double selfUtility = parent.nextPla == P_WHITE ? utilityAvg : -utilityAvg;
