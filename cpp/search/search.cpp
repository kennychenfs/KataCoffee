...
Search::Search(SearchParams params, NNEvaluator* nnEval, Logger* lg, const string& rSeed)
  : rootPla(P_BLACK),
    ...
    rootHistory(),
    rootGraphHash(),
    rootHintLoc(Loc(Board::NULL_LOC, D_NONE)),
...
void Search::setRootHintLoc(Loc loc) {
  ...
  // to happen
  if(loc != Loc(Board::NULL_LOC, D_NONE) && rootHintLoc != loc)
    clearSearch();
  rootHintLoc = loc;
}
    ...
    if(child == NULL) {
      ...
      // Account for time carried over
      {
        ...
        if(visitProportion > 1)
          visitProportion = 1;
        effectiveSearchTimeCarriedOver =
          effectiveSearchTimeCarriedOver * visitProportion * searchParams.treeReuseCarryOverTimeFactor;
      }
...
void Search::beginSearch(bool pondering) {
  ...
  // cout << "BEGINSEARCH " << GameIO::playerToString(rootPla) << " " << GameIO::playerToString(plaThatSearchIsFor) <<
  // endl;
  ...
  if(rootNode == NULL) {
    ...
    if(childrenCapacity > 0 && children != NULL) {
      ...
      // okay. Also we can't be affecting the tree since the root node isn't in the table and can't be transposed to.
      ...
      vector<SearchNode*> filteredNodes;
      {
        ...
        for(; i < childrenCapacity; i++) {
          ...
          children[i].setMoveLoc(Loc(Board::NULL_LOC, D_NONE));
    ...
    if(searchParams.subtreeValueBiasFactor != 0 || patternBonusTable != NULL) {
      ...
      if(anyFiltered) {
        // Recursive stats recomputation resulted in us marking all nodes we have. Anything filtered is old now, delete
        // it.
        bool old = true;
        deleteAllOldOrAllNewTableNodesAndSubtreeValueBiasMulithreaded(old);
      }
...
SearchNode* Search::allocateOrFindNode(SearchThread& thread, Player nextPla, Loc bestChildMoveLoc, Hash128 graphHash) {
  ...
  while(true) {
    ...
    if(insertLoc != nodeMap.end() && insertLoc->first == childHash) {
      ...
      // if the node is accessed concurrently by other nodes through the table, we need to make sure these parameters
      // are fully fully-formed before we make the node accessible to anyone.
      ...
      if(searchParams.subtreeValueBiasFactor != 0 && subtreeValueBiasTable != NULL) {
        ...
        if(thread.history.moveHistory.size() >= 2) {
          ...
          if(prevMoveLoc != Loc(Board::NULL_LOC, D_NONE)) {
            child->subtreeValueBiasTableEntry = subtreeValueBiasTable->get(
              getOpp(thread.pla), prevMoveLoc, bestChildMoveLoc, thread.history.getRecentBoard(1));
          }
        }
      }
      ...
      if(patternBonusTable != NULL)
        child->patternBonusHash =
          patternBonusTable->getHash(getOpp(thread.pla), bestChildMoveLoc, thread.history.getRecentBoard(1));
...
// Delete ALL nodes. More efficient than deleteAllOldOrAllNewTableNodesAndSubtreeValueBiasMulithreaded if deleting
// everything. Doesn't clear subtree value bias.
...
bool Search::playoutDescend(SearchThread& thread, SearchNode& node, bool isRoot) {
  ...
  // forceNonTerminal marks special nodes where we cannot end the game. This includes the root, since if we are
  // searching a position we presumably want to actually explore deeper and get a result. Also it includes the node
  // following a pass from the root in the case where we are conservativePass. For friendlyPassOk rules, it may include
  // deeper nodes. Note that we also carefully clear the search when a pass from the root would be terminal, so nodes
  // should never need to switch status after tree reuse in the latter case.
  if(thread.history.isGameFinished) {
    ...
    // This stops a thread from building a silly number of visits to distort MCTS statistics while other threads are
    // stuck on the GPU.
    nnEvaluator->waitForNextNNEvalIfAny();
    double winLossValue = 2.0 * ScoreValue::whiteWinsOfWinner(thread.history.winner) - 1;
    double weight =
      (searchParams.useUncertainty && nnEvaluator->supportsShorttermError()) ? searchParams.uncertaintyMaxWeight : 1.0;
  ...
  while(true) {
    ...
    // there's an actual hash collision). Regenerate the neural net call and continue Could also be true if we have an
    // illegal move due to graph search and we had a cycle and superko interaction, or a true collision on an older path
    // that results in bad transposition between positions that don't transpose.
    if(bestChildIdx >= 0 && !thread.history.isLegal(thread.board, bestChildMoveLoc, thread.pla)) {
...

      {
        ...
        if(thread.illegalMoveHashes.find(nnHash) == thread.illegalMoveHashes.end()) {
          ...
          out << GameIO::locToString(bestChildMoveLoc, thread.board) << endl;
      ...
      if(bestChildIdx >= 0) {
        ...
        if(bestChildIdx >= numChildrenFound) {
          ...
          // based on an older cached nnOutput that still had the illegal move. If so, then just fail this playout and
          // try again.
        ...
        // Existing child
          ...
          // We want the search to continue as best it can, so we increment visits so other search branches will still
          // make progress.
          int childrenCapacity;
    ...
    if(bestChildIdx <= -1) {
      // This might happen if all moves have been forbidden. The node will just get stuck counting visits without
      // expanding and we won't do any search.
    ...
    if(bestChildIdx >= numChildrenFound) {
      ...
      child = allocateOrFindNode(thread, thread.pla, bestChildMoveLoc, thread.graphHash);
...

      {
        ...
        if(existingChild == NULL) {
          ...
          // Someone got there ahead of us. We already made a move so we can't just loop again. Instead just fail this
          // playout and try again. Even if the node was newly allocated, no need to delete the node, it will get
          // cleaned up next time we mark and sweep the node table later. Clean up virtual losses in case the node is a
          // transposition and is being used.
          child->virtualLosses.fetch_add(-1, std::memory_order_release);
          return false;
        }
      }
...
bool Search::maybeCatchUpEdgeVisits(
  ...
  // If we want to leak through some of the time, then we keep searching the transposition node even if we'd be happy to
  // stop here with how many visits it has
  if(
    searchParams.graphSearchCatchUpLeakProb > 0.0 && edgeVisits < childVisits &&
