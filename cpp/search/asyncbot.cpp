...
Loc AsyncBot::genMoveSynchronous(
  Player movePla,
  const TimeControls& tc,
  double sf,
  const std::function<void()>& onSearchBegun) {
...
void AsyncBot::ponder(double sf) {
  ...
  shouldStopNow = false;
  timeControls = TimeControls();  // Blank time controls since opponent's clock is running, not ours, so no cap other
                                  // than searchFactor
  searchFactor = sf;
  analyzeCallbackPeriod = -1;
  analyzeFirstCallbackAfter = -1;
...
void AsyncBot::analyzeAsync(
  ...
  queuedOnMove = nullptr;
  isRunning = true;
  isPondering = false;  // This should indeed be false because we are searching for the current player, not the last
                        // player we did a regular search for.
  shouldStopNow = false;
  timeControls = TimeControls();  // Blank time controls since no clock is not running, we don't cap search time other
                                  // than through searchFactor.
  searchFactor = sf;
...
void AsyncBot::internalSearchThreadLoop() {
  ...
  while(true) {
    ...
    std::function<void()> searchBegun =
      [this, usingCallbackLoop, &isSearchBegun, &searchBegunCallbackLocal, &callbackLoopWaitingForSearchBegun]() {
    ...
    auto callbackLoop = [this,
                         &isSearchBegun,
                         &callbackLoopWaitingForSearchBegun,
                         callbackPeriod,
                         firstCallbackAfter,
                         &analyzeCallbackLocal,
                         &callbackLoopWaiting,
                         &callbackLoopShouldStop]() {
      ...
      while(true) {
        ...
        callbackLoopWaiting.wait_for(
          callbackLock, std::chrono::duration<double>(periodToWait), [&callbackLoopShouldStop]() {
            return callbackLoopShouldStop.load();
          });
        if(callbackLoopShouldStop.load())
          return;
        callbackLock.unlock();
