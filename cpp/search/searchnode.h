...
struct NodeStats {
  ...
  double getChildWeight(int64_t edgeVisits) { return childWeight(edgeVisits, visits, weightSum); }
};

inline double NodeStatsAtomic::getChildWeight(int64_t edgeVisits) const {
  return NodeStats::childWeight(
    edgeVisits, visits.load(std::memory_order_acquire), weightSum.load(std::memory_order_acquire));
}
...
inline double NodeStatsAtomic::getChildWeightSq(int64_t edgeVisits) const {
  return NodeStats::childWeightSq(
    edgeVisits, visits.load(std::memory_order_acquire), weightSqSum.load(std::memory_order_acquire));
}
...
struct SearchChildPointer {
 private:
  ...
  std::atomic<Loc>
    moveLoc;  // Generally this will be always guarded under release semantics of data or of the array itself.
 public:
  SearchChildPointer();
